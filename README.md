...
4. `qwen-tts-jp-starter` フォルダごと、Mac の好きな場所へコピーします
5. コピー先のフォルダ内にある `かんたんボイスクローン.app` をダブルクリックします
6. 最初だけアプリ内の `初回セットアップ` を押します
7. セットアップ後に、同じアプリ画面の `起動` を押します
...
1. `qwen-tts-jp-starter` フォルダごと Mac の好きな場所へコピーします
2. コピー先の `qwen-tts-jp-starter` フォルダを開きます
3. `かんたんボイスクローン.app` をダブルクリックします
4. 最初はアプリ内の `初回セットアップ` を押します
5. 使うときはアプリ内の `起動` を押します

`.app` だけを単独で移動すると、同梱している `run.command` や設定ファイルを見失って起動できません。必ずフォルダごと使ってください。
