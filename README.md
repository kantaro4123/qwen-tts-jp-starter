...
6. 最初だけアプリ内の `① セットアップ` を押します
7. 準備が終わったら、同じアプリ画面の `② 起動` を押します

DMG 版では、起動後もブラウザではなくアプリ内の画面でそのまま使えます。
...
3. `参照素材を整える` を押して、整えた参照音声を確認します
4. `参照音声の文字起こし` を入力します
5. 必要なら `自動文字起こし` を使います
6. `読ませたい文章` を入力します
7. `読み上げ言語` を選びます
8. `音声を生成` を押します
...
- DMG 版: 初回セットアップで一緒に入ります
- ソース版: `./setup.command` で一緒に入ります
- 入れ直したいときだけ `音声認識を追加（任意）` または `./install_local_asr.command` を使います
...
DMG 版は Python を内蔵しているので、通常は別途 Python を入れなくても使えます。
